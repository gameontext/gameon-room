...
public class Fuse extends ItemDesc {
	...
	public static final ItemUseHandler handler = new ItemUseHandler(){
		private final CommandTemplate useInventoryFuseWithRoomItem =new CommandTemplateBuilder().build(Type.INVENTORY_ITEM).build(Type.LINKWORD,"With").build(Type.CONTAINER_ITEM).build();
			...
			if(u!=null){ 
				...
				if(key.equals(useInventoryFuseWithRoomItem.key)){
					if(jb.item.equals(Items.jukebox)){
						...
						room.playerEvent(execBy, "You try several times to use the "+fuse.item.name+" with the "+jb.item.name+" but can't seem to figure out how.",null);
					}
				}
			}
		}
				...
				if(u!=null){	
					...
					if(next==null){
						...
						if("WITH".equals(next)){
							...
							if(otherItem!=null){
								if(otherItem.equals(Items.jukebox)){
