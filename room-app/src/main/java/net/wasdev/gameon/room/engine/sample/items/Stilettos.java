...
public class Stilettos extends ItemDesc{
	...
	public static ItemUseHandler useHandler = new ItemUseHandler() {
		...
		private final CommandTemplate useStilettosInInventory =new CommandTemplateBuilder().build(Type.INVENTORY_ITEM).build();
		private final CommandTemplate useStilettosWithRoomItem =new CommandTemplateBuilder().build(Type.INVENTORY_ITEM).build(Type.LINKWORD,"With").build(Type.ROOM_ITEM).build();
		...
		private final Set<CommandTemplate> templates = Collections.unmodifiableSet(new HashSet<CommandTemplate>(Arrays.asList(new CommandTemplate[]{
				useStilettosInRoom,
				useStilettosInInventory,
				useStilettosWithRoomItem
		})));
			...
			if(u!=null){ 
				...
				if(key.equals(useStilettosInRoom.key)){
					...
					if(other.item.equals(Items.cupboard)){
						...
						room.playerEvent(execBy, "You try several times to use the stiletto heels with the "+other.item.name+" but can't seem to figure out how.",null);
					}
				}
			}
		}
		...
		public void processUnknown(Room room, String execBy, String origCmd, String cmdWithoutVerb) {
			room.playerEvent(execBy, "That doesn't sound like the way you are supposed to use the " + cmdWithoutVerb, null);
		}
